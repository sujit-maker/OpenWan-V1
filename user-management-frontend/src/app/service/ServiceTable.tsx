...
import { Service } from "./types"; 
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this service?")) {
      try {
        const response = await fetch(`http://40.0.0.109:8000/services/${id}`, {
          method: "DELETE",
        });
        if (!response.ok) {
          throw new Error("Network response was not ok");
        ...
        <table className="min-w-full border-collapse bg-white shadow-lg rounded-lg ml-6">
          ...
          <tbody>
            {filteredServices.map((service) => (
              <tr key={service.id}>
                <td className="border p-2 text-center">{service.serviceId}</td>
                <td className="border p-2 text-center">{service.serviceName}</td>
                <td className="border p-2 text-center">{service.description}</td>
