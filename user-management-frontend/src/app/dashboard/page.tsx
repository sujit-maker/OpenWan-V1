    ...
    const fetchUserCounts = async () => {
      try {
        ...
        if (superadminId) {
          ...
          const allCountsResponse = await fetch(`http://40.0.0.25:8000/users/counts`);
        ...
        if (managerId) {
          const response = await fetch(`http://40.0.0.25:8000/users/count/manager/${managerId}`);
        ...
        if (adminId) {
          ...
          const adminResponse = await fetch(`http://40.0.0.25:8000/users/count/admin/${adminId}`);
          ...
          const executiveResponse = await fetch(`http://40.0.0.25:8000/users/count/executives/admin/${adminId}`);
